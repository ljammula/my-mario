...
// Maps button id → key code(s) to activate
const TOUCH_BTN_MAP = {
  'btn-left':  ['ArrowLeft'],
  'btn-right': ['ArrowRight'],
  'btn-a':     ['Space'],              // Jump
  'btn-b':     ['ShiftLeft', 'KeyX'],  // Run (ShiftLeft) + Fire (KeyX)
  'btn-start': ['Enter'],              // Pause / Start
};
...
function deactivateBtn(btnId) {
  const codes = TOUCH_BTN_MAP[btnId];
  if (!codes) return;
  // Only release if no other active touch is holding this button
...
(function initTouchControls() {
  ...
  // Prevent scroll/zoom on canvas itself
